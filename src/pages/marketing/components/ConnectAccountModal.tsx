...
interface ConnectAccountModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConnect: (accountData: any) => Promise<void>; // Updated to Async
}
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await onConnect({
                platform,
                username,
                password, // Will be encrypted by backend Vault
                product: productSearch, // Use productSearch as the product
                status: 'connected'
            });
            ...
            toast.error("Falha ao conectar a conta.");
        } finally {
            setIsLoading(false);
        }
    };
