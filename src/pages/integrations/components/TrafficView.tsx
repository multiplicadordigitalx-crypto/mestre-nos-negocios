...
export const TrafficView: React.FC = () => {
    ...
    useEffect(() => { 
        ...
        getAdminIntegrations('traffic').then(setAccounts); 
    }, []);
    ...
    const handleAdd = async () => {
        ...
        await saveAdminIntegration('traffic', newItem);
    ...
    const handleDelete = async (id: string) => {
        if(confirm("Remover conta de tráfego?")) { await deleteAdminIntegration('traffic', id); setAccounts(prev => prev.filter(a => String(a.id) !== String(id))); toast.success("Conta removida."); }
    };
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {PLATFORM_OPTIONS.map(opt => (
                                    <button 
                                        ...
                                        className={`flex items-center justify-center gap-2 py-2 rounded-lg border text-xs font-bold transition-all ${
                                            newAccount.platform === opt.id 
