...
export const AIBrainsView: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const toastId = "ai-brains-load-error";
            if (error instanceof Error && (
                error.message.includes('permission-denied') ||
                error.message.includes('insufficient permissions') ||
                error.message.toLowerCase().includes('permission')
            )) {
                toast.error("Acesso restrito: Sem permissão para ler Cérebro IA", { id: toastId });
            } else {
                toast.error("Erro ao carregar configurações de IA", { id: toastId });
            }
        } finally {
            setIsLoading(false);
        }
    ...
    const handleRemoveAi = async (id: string) => {
        if (confirm("Tem certeza que deseja remover esta IA?")) {
            try {
                ...
                toast.error("Erro ao remover IA");
            }
        }
    };

    const handleUpdateAi = async (id: string, field: string, value: string) => {
        const updatedConfigs = aiConfigs.map(ai => {
            if (ai.id === id) {
                const newAi = { ...ai, [field]: value };
                if (field === 'key') {
                    newAi.status = value.trim().length > 5 ? 'connected' : 'missing';
                }
                return newAi;
            }
            return ai;
        ...
        const targetAi = updatedConfigs.find(ai => ai.id === id);
        if (targetAi) {
            try {
                await saveAIConfig(targetAi);
            } catch (error) {
