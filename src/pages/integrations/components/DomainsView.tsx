...
export const DomainsView: React.FC = () => {
    ...
    useEffect(() => { 
        getAdminIntegrations('domain').then(setProviders); 
    }, []);
    ...
    const handleAddProvider = async () => {
        ...
        const newItem = { id: Date.now(), name: newProvider.name, type: newProvider.type, status: 'active', domain: newProvider.domain, dnsRecords: 0, product: newProvider.product };
        await saveAdminIntegration('domain', newItem);
    ...
    const handleDelete = async (id: number) => {
        if(confirm("Desconectar domínio? Isso pode afetar páginas ativas.")) {
            await deleteAdminIntegration('domain', id);
