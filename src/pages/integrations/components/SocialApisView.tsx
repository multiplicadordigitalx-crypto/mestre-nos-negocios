...
export const SocialApisView: React.FC<{ isAdmin?: boolean }> = ({ isAdmin }) => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const data = await getSocialApis();
            setIntegrations(data || []);
        } catch (error) {
            console.error("Error loading social APIs:", error);

            // Fixed ID prevents duplicates if called multiple times (e.g. React StrictMode)
            const toastId = "social-apis-load-error";
            ...
            if (error instanceof Error && (
                ...
                toast.error("Acesso restrito: Sem permissão para ler APIs sociais", { id: toastId });
            } else if (error instanceof Error) {
                toast.error("Erro ao carregar APIs sociais: " + error.message, { id: toastId });
            } else {
                toast.error("Erro ao carregar APIs sociais", { id: toastId });
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast.error("Erro ao salvar integração social");
        }
    };
