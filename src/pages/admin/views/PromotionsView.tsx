...
import { 
    Percent, PlusCircle, Trash, Copy, Calendar, Tag, 
    ShoppingBag, CheckCircle, X as XIcon, Save, Zap 
...
interface Coupon {
    ...
    product: string; // 'All' or specific Product Name
    uses: number;
...
const PromotionsView: React.FC = () => {
    const [coupons, setCoupons] = useState<Coupon[]>([
        ...
        { id: '3', code: 'VIP_LANCAMENTO', discount: 20, type: 'percentage', product: 'Mentoria Elite', uses: 12, limit: 50, expiration: '2024-06-30', status: 'active' },
    ]);
    ...
    const handleSave = () => {
        if (!newCoupon.code || !newCoupon.discount || !newCoupon.expiration) {
        ...
        const coupon: Coupon = {
            ...
            type: 'percentage',
            product: newCoupon.product || 'Todos',
            uses: 0,
        ...
        setNewCoupon({ code: '', discount: 10, type: 'percentage', product: 'Todos', limit: 100, expiration: '' });
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-2 uppercase tracking-wide">
                        <Percent className="w-8 h-8 text-pink-500"/> Gestão de Ofertas & Cupons
                    </h2>
                    <p className="text-gray-400 text-sm mt-1">Crie códigos promocionais e gerencie campanhas de desconto.</p>
                </div>
                <Button onClick={() => setIsModalOpen(true)} className="!bg-pink-600 hover:!bg-pink-500">
                    <PlusCircle className="w-5 h-5 mr-2"/> Novo Cupom
                </Button>
            </div>
                        ...
                        <div className="p-6 bg-gradient-to-br from-gray-800 to-gray-900">
                            ...
                            <div className="flex items-center gap-2 mb-6">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xs font-bold text-white uppercase">OFF</span>
                                    <span className="text-[10px] text-gray-500">Desconto</span>
                                </div>
                            </div>

                            <div className="space-y-2 text-xs text-gray-400 border-t border-gray-700/50 pt-4 border-dashed">
                                <div className="flex justify-between">
                                    <span className="flex items-center gap-1"><ShoppingBag className="w-3 h-3"/> Produto:</span>
                                    <span className="text-white font-medium">{coupon.product}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="flex items-center gap-1"><Tag className="w-3 h-3"/> Usos:</span>
                                    <span className="text-white font-medium">{coupon.uses} / {coupon.limit === null ? '∞' : coupon.limit}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="flex items-center gap-1"><Calendar className="w-3 h-3"/> Expira em:</span>
                                    <span className="text-white font-medium">{new Date(coupon.expiration).toLocaleDateString()}</span>
                                </div>
                            </div>

                            <div className="mt-4 pt-4 border-t border-gray-700/50 flex justify-end">
                                <button onClick={() => handleDelete(coupon.id)} className="text-red-400 hover:text-red-300 p-2 rounded hover:bg-red-900/10 transition-colors">
                    ...
                    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[100] p-4">
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-gray-800 w-full max-w-lg rounded-2xl border border-pink-500/30 shadow-2xl p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs text-gray-400 font-bold uppercase mb-1 block">Limite de Uso</label>
                                        <input 
                                            type="number" 
                                            ...
                                            onChange={e => setNewCoupon({...newCoupon, limit: e.target.value ? Number(e.target.value) : null})}
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <select 
                                            ...
                                            onChange={e => setNewCoupon({...newCoupon, product: e.target.value})}
                                        >
                                            <option value="Todos">Todos os Produtos</option>
                                            <option value="Mestre 15X">Mestre 15X</option>
                                            <option value="Mentoria Elite">Mentoria Elite</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
