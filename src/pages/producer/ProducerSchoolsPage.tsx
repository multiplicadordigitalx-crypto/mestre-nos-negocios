    ...
    useEffect(() => {
        const loadSchools = async () => {
            if (user) {
                ...
                // Filter logic:
                // 1. Must belong to user (mock: assumes all for now, or filter by ownerId if available)
                // 2. Drafts (no financialViability) are hidden from here (they stay in Wizard/AdminTrainingPage)

                // For MVP mock, assuming all courses in mockFirebase belong to the current producer session context
                // Real app: .filter(c => c.ownerId === user.uid)

                const validSchools = allCourses.filter(c => c.financialViability); // Only show if Wizard Step 7 completed
                setSchools(validSchools);
            }
        };
        loadSchools();
    }, [user, schoolForChecklist]); // Reload when checklist closes to update progress

    // Filter by Tab
    const displayedSchools = schools.filter(s => {
        if (activeTab === 'published') return s.isPublished;
        if (activeTab === 'setup') return !s.isPublished; // Completed Wizard but not Published
        return false;
    });
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
                    ...
                    <div className="bg-gray-900/50 p-1 rounded-xl flex gap-1 border border-gray-800">
                        ...
                        >
                            ...
                            Ativas
                            <span className="bg-gray-900 px-1.5 py-0.5 rounded text-[10px] text-gray-400 font-mono">{schools.filter(s => s.isPublished).length}</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            Em Setup
                            <span className="bg-gray-900 px-1.5 py-0.5 rounded text-[10px] text-gray-400 font-mono">{schools.filter(s => !s.isPublished && s.financialViability).length}</span>
                        </button>
                    </div>
                </div>
                ...
                {displayedSchools.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {displayedSchools.map(school => {
                            const config = getSchoolConfigByNiche(school.niche || 'Generic');
                            const primaryColor = config.theme?.primaryColor || '#FACC15';
                            const isSetup = !school.isPublished;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {!isSetup && (school as any).consultancyStatus !== 'monitoring' && (
                                        <div
                                    ...
                                    <div className="p-6 relative">
                                        ...
                                        <div className="mt-4 mb-6">
                                            <h4 className="text-lg font-black text-white leading-tight mb-1 truncate">{school.title}</h4>
                                        ...
                                        {isSetup ? (
                                            <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700 mb-4">
                                                ...
                                                <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                                                    <div className="h-full bg-brand-primary transition-all duration-500" style={{ width: `${school.setupProgress || 0}%` }}></div>
                                                </div>
